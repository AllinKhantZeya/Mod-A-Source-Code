...
public class DragonIngotMaker extends Block{

	public DragonIngotMaker(Material materialIn) {
		super(materialIn);
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ)
    {
		...
		if (item == Items.iron_ingot || item == Items.gold_ingot || item == ModAItems.dark_ingot || item == ModAItems.wither_ingot){
			playerIn.inventory.consumeInventoryItem(item);
			Functions.spawnItemOnBlock(worldIn, ModAItems.dragon_ingot, 1, pos);
			return false;
		}
		else{
